    ...
    public DefaultExceptionCodes() {
        ErrorHandler.defaultInstance()
            .onError(SQLException.class, ((throwable, errorHandler) -> {
                if (CloudAPI.getInstance().getLogger() == null) {
                    System.err.println("SQLError occurred, check your database credentials! (" + throwable.getMessage() + ")");
                    return;
                }
                CloudAPI.getInstance().getLogger().logMessage("§cSQLError occurred§7, check your §bdatabase credentials! §7(§b" + throwable.getMessage() + "§7)", LogType.ERROR);
            }))
            .bindCode(1, errorCode -> throwable -> throwable instanceof RuntimeException, ((throwable, errorHandler) -> {
                ...
                CloudAPI.getInstance().getLogger().logMessage("§cFatal exception occurred§7, cloud will try to stop... (§b" + throwable.getMessage() + "§7)", LogType.ERROR);
