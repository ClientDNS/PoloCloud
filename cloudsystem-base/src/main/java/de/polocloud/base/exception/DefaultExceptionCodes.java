    ...
    public DefaultExceptionCodes() {
        ErrorHandler.defaultInstance()
            .onError(SQLException.class, ((throwable, errorHandler) -> {
                ...
                CloudAPI.getInstance().getLogger().log("§cSQLError occurred§7, check your §bDatabase credentials! §7(§b" + throwable.getMessage() + "§7)", LogType.ERROR);
            }))
            .bindCode(1, errorCode -> throwable -> throwable instanceof RuntimeException, ((throwable, errorHandler) -> {
                ...
                CloudAPI.getInstance().getLogger().log("§cFatal exception occurred§7, cloud will try to stop... (§b" + throwable.getMessage() + "§7)", LogType.ERROR);
